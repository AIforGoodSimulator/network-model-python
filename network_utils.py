...
def link_nodes_by_activity(base_graph, nodes_per_struct, percentage_per_struct, proximity_radius, edge_weight, activity_name):
    ...
    for grid in grids:
        ...
        for i in range(len(subgrids)):
            ...
            graph = connect_food_queue(
                graph, nodes_per_struct_subgrid, food_weight, f"food_{queue_num}")
            queue_num += 1
